    ...
    .post(cors(), async function (req, res) {
        try {
            ...
            if(!await Logins.canAuthenticate(identityKey)){
                return res.status(500).send("The account is temporarily locked out.");
            }
            ...
            console.log(err);
            return res.status(500).send("There was an error attempting to login. Please try again later.");
        }
    });
