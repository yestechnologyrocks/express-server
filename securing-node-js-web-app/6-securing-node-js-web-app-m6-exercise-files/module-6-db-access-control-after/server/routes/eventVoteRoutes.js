...
eventVoteRouter.route("/api/event/vote")
    .get(cors(), async function (req, res) {
        try {
            const TimelineItem = await getTimelineItemModel("timelineItem", "query");
...
eventVoteRouter.route("/api/event/vote")
    .post(cors(), async function (req, res) {
        try {
            ...
            const user = await UserQuery.findOne({_id: userInfo._id}).exec();
            if (!user) {
            ...
            const existingVote = await EventVoteQuery.findOne(eventVote).exec();
